...
public class GameMain {
    ...
    private static void createAndShowGUI() {
        ...
        JMenuBar jay = new JMenuBar();
        JMenu j = new JMenu("File");
        j.add(saveItem);
        j.add(exitItem);
        jay.add(j);
        frame.setJMenuBar(jay);
    ...
    private static JComponent makeSliderPanel(JSlider slider, String title) {
        ...
        JPanel jesus = new JPanel(new BorderLayout());
        jesus.setBorder(BorderFactory.createEmptyBorder(4, 4, 4, 4));

        JLabel lab = new JLabel(title);
        lab.setFont(lab.getFont().deriveFont(16.0f));
        jesus.add(lab, BorderLayout.NORTH);
        jesus.add(slider);

        return jesus;  // Replace this line with one that returns your panel
    }
    ...
    private static void saveScore(JFrame frame, int score) {
        ...
        final JFileChooser jeff = new JFileChooser();
        jeff.showSaveDialog(frame);

        try (PrintWriter out = new PrintWriter(new BufferedWriter
                (new FileWriter(jeff.getSelectedFile(), true)))) {
            out.println(score);
        } catch (IOException e) {
            String eleanor = e.toString();
            JOptionPane.showMessageDialog(frame, "suck my peepee", eleanor, JOptionPane.ERROR_MESSAGE);
        }
