...
public class GameComponent extends JPanel implements MouseListener {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (!isActive) g.fillRect(0,0,getWidth(),getHeight());
    ...
    private static class Target {
        ...
        void paintDot(Graphics g) {
            ...
            if (!isHit) g.setColor(Color.blue);
            else if (isHit) g.setColor(Color.red);
        ...
        boolean checkHit(int cx, int cy) {
            ...
            //from now on, we know it has not yet been hit
            double changeinX = cx-x;
            double changeinY = cy-y;
            double comp = Math.pow(changeinX,2);
            double comp2 = Math.pow(changeinY,2);
            double sqrt = Math.sqrt(comp+comp2);
            if (sqrt<=radius) {
                isHit = true;
                return true;
            }
