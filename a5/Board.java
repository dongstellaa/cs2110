...
public class Board {
    ...
    public boolean equals(Object o) {
        ...
        if (((Board) o).colSize != colSize()) return false;
        if (((Board) o).rowSize != rowSize()) return false;
        for (int i = 0; i < ((Board) o).boardState.length; i++){
            if (((Board) o).boardState[i] != boardState[i]) return false;
        }
        return true;
    }
    ...
    public int hashCode() {
        ...
        int ret = Arrays.hashCode(boardState);
        ret = (ret*101) + Objects.hash(rowSize, colSize);
        return ret;
    }
